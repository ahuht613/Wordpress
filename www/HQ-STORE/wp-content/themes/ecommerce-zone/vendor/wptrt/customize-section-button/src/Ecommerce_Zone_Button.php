...
class Ecommerce_Zone_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $ecommerce_zone_theme->get( 'ThemeURI' ) ) {

			$button_url = $ecommerce_zone_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $ecommerce_zone_theme->get( 'AuthorURI' ) ) {

			$button_url = $ecommerce_zone_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $ecommerce_zone_theme->get( 'Name' );
