 ...
 * Register widget
 */
function shop_hub_register_widget() {
	register_widget( 'Shop_Hub_Social_Icons_Widget' );
}
add_action( 'widgets_init', 'shop_hub_register_widget' );
 ...
 * @since 1.0.0
 */
class Shop_Hub_Social_Icons_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			// widget ID.
			'shop_hub_social_icons',
			// widget name.
			__( 'Shop Hub Social Icons', 'shop-hub' ),
			// widget description.
			array( 'description' => __( 'Adds a social icon menu.', 'shop-hub' ) )
		);
	}
	...
	public function form( $instance ) {
		...
		if ( ! $menus ) {
			...
			echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'shop-hub' ), esc_url( admin_url( 'nav-menus.php' ) ) ) . '</p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return;
		}
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'shop-hub' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>"><?php esc_html_e( 'Select Menu:', 'shop-hub' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'nav_menu' ) ); ?>">
				<option value="0"><?php esc_html_e( '&mdash; Select &mdash;', 'shop-hub' ); ?></option>
				<?php
